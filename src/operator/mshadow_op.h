...
#define MXNET_SIMPLE_UNARY_MATH_OP(name) \
  MXNET_UNARY_MATH_OP(name, math::name(a))

#define MXNET_SIMPLE_BINARY_MATH_OP(name) \
  MXNET_BINARY_MATH_OP(name, math::name(a, b))
...
struct assign_1 : public mxnet_op::tunable {
  template <typename DType>
  MSHADOW_XINLINE static DType Map(DType a) {
    return DType(1.0f);
  }
...
struct assign_0 : public mxnet_op::tunable {
  template <typename DType>
  MSHADOW_XINLINE static DType Map(DType a) {
    return DType(0.0f);
  }
...
MXNET_UNARY_MATH_OP(erfinv_grad,
                    0.5 * math::sqrt(PI) *
...
MXNET_UNARY_MATH_OP(reciprocal_square_root_grad,
                    -0.5f / (math::sqrt(a) * math::id(a)));
...
MXNET_UNARY_MATH_OP(reciprocal_cube_root_grad,
                    -1.0f / (3.0f * math::cbrt(a) * math::id(a)));
...
struct fix : public mxnet_op::tunable {
  ...
  MSHADOW_XINLINE static DType Map(DType a) {
    auto floor = math::floor(a);
    auto ceil = math::ceil(a);
    return DType((floor > 0 ? floor : -floor) < (ceil > 0 ? ceil : -ceil)
                     ? floor
                     : ceil);
  }
};
...
MXNET_BINARY_MATH_OP(copysign_grad,
                     (a >= 0 && b >= 0) || (a < 0 && b < 0) ? 1 : -1);

MXNET_BINARY_MATH_OP(copysign_rgrad, 0);

MXNET_BINARY_MATH_OP(rcopysign,
                     (b >= 0 && a >= 0) || (b < 0 && a < 0) ? b : -b);
...
#else
  return mshadow::half::half2_t(
      mshadow::half::half_t(
          -::floorf(static_cast<float>(a.half_t2[0] / b.half_t2[0]))),
      mshadow::half::half_t(
          -::floorf(static_cast<float>(a.half_t2[1] / b.half_t2[1]))));
#endif
}
...
MSHADOW_XINLINE mshadow::half::half2_t rmod_grad::Map<mshadow::half::half2_t>(
  ...
  return mshadow::half::half2_t(
      mshadow::half::half_t(
          -::floorf(static_cast<float>(b.half_t2[0] / a.half_t2[0]))),
      mshadow::half::half_t(
          -::floorf(static_cast<float>(b.half_t2[1] / a.half_t2[1]))));
#endif
}
...
struct product {
  ...
  /*! \brief combine the results of two reducers */
  template <typename DType>
  MSHADOW_XINLINE static void Merge(
  ...
  /*! \brief finalize reduction */
  template <typename DType>
  MSHADOW_XINLINE static void Finalize(volatile DType& dst,
                                       volatile DType& none) {}  // NOLINT(*)
  /*!
...
MXNET_UNARY_MATH_OP_NC(relu,
                       isnan_typed::IsNan(a) || (a > DType(0)) ? a : DType(0));
...
struct nansum {
  ...
  MSHADOW_XINLINE static void Merge(
    ...
    DType e = t1 - src_val;
    DType t2 =
        ((src_val - e) + (dst_val - (t1 - e))) + dst_residual + src_residual;
    dst_val = t1 + t2;
...
struct nrm2 {
  ...
  MSHADOW_XINLINE static void Merge(volatile DType& dst_ssq,
    ...
    if (dst_scale != 0 && dst_scale >= src_scale) {
      dst_ssq =
          dst_ssq + src_ssq * (src_scale / dst_scale) * (src_scale / dst_scale);
    } else if (src_scale != 0 && dst_scale < src_scale) {
      dst_ssq =
          src_ssq + dst_ssq * (dst_scale / src_scale) * (dst_scale / src_scale);
      dst_scale = src_scale;
    }
  }
  /*! \brief finalize reduction result */
  template <typename DType>
  MSHADOW_XINLINE static void Finalize(
      volatile DType& sum_of_squares) {  // NOLINT(*)
   ...
   */
  template <typename DType>
  MSHADOW_XINLINE static void SetInitValue(
      DType& sum_of_squares) {  // NOLINT(*)
    sum_of_squares = 0;
  }
  /*!
   *\brief set the initial value during reduction
...
struct lcm : public mxnet_op::tunable {
  template <typename DType>
  MSHADOW_XINLINE static
      typename enable_if<is_integral<DType>::value, DType>::type
      Map(DType a, DType b) {
    // minus cases.
    if (a < 0) {
      a = -a;
